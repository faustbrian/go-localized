...
.PHONY: api-check benchmark check coverage docs format format-check fuzz lint \
	mutation nilaway-advisory postgres postgres-matrix safety standards test \
	test-race vet vuln workflow-lint
...
nilaway-advisory:
	...
	@if $(GO) run go.uber.org/nilaway/cmd/nilaway@v0.0.0-20260710181136-2378218750e4 \
		-include-pkgs='github.com/faustbrian/go-localized/...' ./...; then \
