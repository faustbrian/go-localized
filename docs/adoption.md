...
For go-http-client, use `localizedhttpclient.WithPreferences` on the desired
...
See [API integration examples](api-integrations.md) for concrete go-jsonapi,
go-jsonrpc, and OpenRPC shapes.

## Configuration

Use `localizedconfig.Text` for a presence-aware go-config field. `Valid=false`
...
Use `localizedvalidation.Validator(rules...)` inside a `go-validation` plan.
...
in the owning `go-api-query` compiler configuration.
