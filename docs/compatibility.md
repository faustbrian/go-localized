...
| `go-international/locale` | `v0.0.0-20260717012043-f6e9bbc622bd` | public BCP 47 identity and provenance |
| `golang.org/x/text` | v0.40.0 | private CLDR matching and Unicode normalization |
| pgx | v5.10.0 | JSONB and PostgreSQL integration |
| go-wire | pinned pseudo-version in `go.mod` | bounded format adapters |
| go-config | pinned pseudo-version in `go.mod` | configuration hook conformance |
