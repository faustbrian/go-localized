...
- `Text`: immutable canonical entries; zero value is empty and concurrent-read
  safe.
- `Entry`: typed `go-international/locale.Tag` and UTF-8 `Text` pair.
...
- `Validator`: typed `go-validation` adapter with canonical locale-key paths.
...
`go-wire` packages and revalidate localized ownership.

## `localizedconfig`

- `Text`, `NewText`: present/null wrapper.
- `ConfigTextTarget`, `UnmarshalConfigValue`: go-config structural hooks.
...
- `WithPreferences`: write or remove a canonical bounded `Accept-Language`
  header on an immutable go-http-client request spec.
