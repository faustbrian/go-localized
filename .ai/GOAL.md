...
- `go-international` owns locale primitives and registry data.
...
- Integrate with `go-international` locale types backed by BCP 47 and the IANA
...
- Text, JSON, YAML, TOML, and MessagePack integration belongs through `go-wire`
...
- `go-wire`, `go-validation`, `go-config`, and `go-api-query` adapters.
- `go-http-client` adapter for `Accept-Language` negotiation and response
  selection where useful, without coupling core values to HTTP.
- `go-jsonapi`, `go-jsonrpc`, and `go-openrpc` examples for localized fields and
...
- No ownership of BCP 47 parsing or registry lifecycle when `go-international`
